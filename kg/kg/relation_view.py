...
def sortDict(relationDict):
	for i in range( len(relationDict) ):
		relationName = relationDict[i]['rel']['name']  #type
...
def  search_entity(request):
	...
	if(request.GET):
		...
		if len(entityRelation) == 0:
			...
			return render(request,'knowledge.html',{'entityRelation':json.dumps(entityRelation,ensure_ascii=False)})
	...
	if(request.GET):
		...
		if(len(entity1) != 0 and len(relation) == 0 and len(entity2) == 0):
			...
			if(len(searchResult)>0):
				return render(request,'Relationship between.html',{'searchResult':json.dumps(searchResult,ensure_ascii=False)})
		...
		if(len(entity1) !=0 and len(relation) == 0 and len(entity2)!=0):
			...
			searchResult = sortDict(searchResult)
			if (len(searchResult) > 0):
				return render(request, 'Relationship between.html',
		...
		return render(request,'Relationship between.html',{'ctx':ctx})
